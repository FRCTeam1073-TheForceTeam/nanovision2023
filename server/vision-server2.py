...
capture_pipeline = "v4l2src device=%s ! video/x-raw,format=(string)UYVY,width=(int)1280,height=(int)720,framerate=60/1 ! videorate max-rate=30 ! videoscale ! video/x-raw,width=(int)320,height=(int)180 ! videoflip method=rotate-180 ! videoconvert ! video/x-raw,format=(string)BGR ! appsink emit-signals=True drop=true"%(captureDevice)
...
output = cv2.VideoWriter(output_pipeline, cv2.CAP_GSTREAMER, 30, (320, 180))
